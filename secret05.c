...
int main(void) {
  ...
  char *vertices_to_add[NUM_VERTICES]= {
    "koala", "panda", "snake", "salamander", "gecko", "elephant", "dog",
   ...
   */
  const int edge_weight_values[NUM_VERTICES][NUM_VERTICES]= {
  ...
  /* add the edge_weight_values */
  for (i= 0; i < NUM_VERTICES; i++)
    for (j= 0; j < NUM_VERTICES; j++)
      if (edge_weight_values[i][j] != 0)
        create_graph_edge(&graph, vertices_to_add[i], vertices_to_add[j],
                 edge_weight_values[i][j]);

  /* check the vertices */
  assert(vertex_count(&graph) == 30);

  for (i= 0; i < NUM_ELTS(vertices_to_add); i++)
    assert(isa_vertex(&graph, vertices_to_add[i]));

  /* check the edge_weight_values */
  for (i= 0; i < NUM_VERTICES; i++)
    for (j= 0; j < NUM_VERTICES; j++)
      if (edge_weight_values[i][j] != 0)
